...
int max_transactions=10; // MAX TRANSACTIONS 1000
int max_num=10;     
unsigned int start_transaction_num=3;
int write_size_min=5000;
int write_size_max=10000;
int write_size;
int data_read = 5000;
int num_commits = 0;
int num_aborts = 0;
...
{
  int count, count_val, r_num, iter_num =0;
  int write_diff = write_size_max-write_size_min;
  fstream traceFileStream;
  char *traceFileName = (char *)(malloc)(100 * sizeof (char));
  ...
  for(;iter_num<num_iters; iter_num++)
  {
	  ...
	  num_aborts=0;
	  num_commits=0;	
	  sprintf (traceFileName, "trace%d.txt", iter_num);
	 ...
	 for(count=1; count<max_transactions; count++)
	 {
		  ...
		  traceFileStream << curr_transaction_num << endl;
		  write_size = 	write_size_min + (rand() % write_diff);
